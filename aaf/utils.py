    ...
    def __init__(self, names, packageName):
        ...
        Form.__init__(self, ("STARTITEM 0\n"
                             ...
                             "\n"
                                 ...
                                 "killOptions": Form.ChkGroupControl(["kill"])})
        self.pns = names
        self.pn = packageName

    def show(self, idaDebugPort, debug, iskill):
        self.Compile()
        self.idaDebugPort.value = idaDebugPort
        self.debug.checked = debug
        self.kill.checked = iskill
        if self.Execute() != 1:
            return
        return (self.pns[self.names.value], self.idaDebugPort.value, self.debug.checked,self.kill.checked)

    def OnButtonNop(self, code=0):
        print self.pns[self.names.value]
        pass
